...
const TimerEngine = (() => {
  ...
  function _updateUI() {
    ...
    const currentTaskEl = document.getElementById('dashboard-current-task');
    if (currentTaskEl) {
      if (AppState.timerMode === 'work' && AppState.activeTaskId) {
        try {
  ...
  function _onComplete() {
    ...
    // Send browser notification and audio
    if (completedMode === 'work') {
    ...
    // Auto-start next session after 3 seconds if setting is enabled
    const willAutoStart = (completedMode === 'work') ? AppState.settings.autoStartBreaks : AppState.settings.autoStartFocus;
    if (willAutoStart) {
  ...
  return { start, pause, reset, skip, updateUI: _updateUI, onTick, onSessionComplete, getProgress };
})();
...
const AudioEngine = (() => {
  ...
  // Tibetan Bowl generic chime
  const _chime = new Audio('https://cdn.freesound.org/previews/339/339809_5121236-lq.mp3');
...
function initSettingsPage() {
  ...
  const toggleAutoBreak = el('toggle-autostart-break');
  const toggleAutoFocus = el('toggle-autostart-focus');
  if (toggleNotif) toggleNotif.checked = s.notificationsEnabled;
  if (toggleSound) toggleSound.checked = s.soundEnabled;
  if (toggleAutoBreak) toggleAutoBreak.checked = s.autoStartBreaks;
  if (toggleAutoFocus) toggleAutoFocus.checked = s.autoStartFocus;
  ...
  if (saveBtn) {
    ...
    freshSave.addEventListener('click', () => {
      ...
      const newAutoBreak = el('toggle-autostart-break')?.checked ?? s.autoStartBreaks;
      const newAutoFocus = el('toggle-autostart-focus')?.checked ?? s.autoStartFocus;
      ...
      AppState.settings.autoStartBreaks = newAutoBreak;
      AppState.settings.autoStartFocus = newAutoFocus;
...
const AmbienceMixer = (() => {
  ...
  const AUDIO_URLS = {
    'rain': 'https://cdn.freesound.org/previews/332/332250_5450487-lq.mp3',
    'ocean': 'https://cdn.freesound.org/previews/157/157880_177850-lq.mp3',
    'cafe': 'https://cdn.freesound.org/previews/504/504900_9829875-lq.mp3',
    'campfire': 'https://cdn.freesound.org/previews/729/729395_12863902-lq.mp3',
    'birds': 'https://cdn.freesound.org/previews/427/427400_5228642-lq.mp3',
    'keyboard': 'https://cdn.freesound.org/previews/700/700412_14828549-lq.mp3',
    'wind': 'https://cdn.freesound.org/previews/212/212943_1648170-lq.mp3',
    'night': 'https://cdn.freesound.org/previews/521/521843_129727-lq.mp3',
    // Fallbacks for any mislabeled keys
    'waves': 'https://cdn.freesound.org/previews/157/157880_177850-lq.mp3',
    'fire': 'https://cdn.freesound.org/previews/729/729395_12863902-lq.mp3'
  };

  function _getSliderValue(type) {
    const slider = document.querySelector(`.ambience-slider[data-sound="${type}"]`);
    return slider ? slider.value / 100 : 0.5;
  }

  function _setVolume(type, vol) {
    if (_nodes[type]) {
      _nodes[type].volume = vol;
    }
  }
  ...
  function _toggleSound(type) {
    if (_nodes[type]) {
      ...
      // Start
      if (!AUDIO_URLS[type]) return false;
  ...
  function _loadState() {
    try {
      ...
      Object.entries(state).forEach(([type, s]) => {
        ...
        // Don't auto-play on load due to browser restrictions
      });
    } catch (e) { }
  }
...
const TodoPage = (() => {
  ...
  function init() {
    ...
    [dailyList, futureList].forEach(list => {
      ...
      list.addEventListener('click', (e) => {
        ...
        if (focusBtn) {
          const taskId = parseInt(focusBtn.dataset.id);
  ...
  return { init, render, completeTaskById };
})();
...
const ZenModeController = (() => {
  ...
  function init() {
    ...
    const btnExit = document.getElementById('btn-exit-zen');
    if (btnZen) btnZen.addEventListener('click', toggle);
    if (btnExit) btnExit.addEventListener('click', exit);
...
/* =============================================
   TASK COMPLETION MODAL CONTROLLER
   ============================================= */
const TaskCompletionController = (() => {
  function init() {
    const modal = document.getElementById('task-completion-modal');
    const btnDone = document.getElementById('btn-task-done');
    const btnNotYet = document.getElementById('btn-task-not-yet');
    ...
    if (btnNotYet) btnNotYet.addEventListener('click', hide);
    if (btnClose) btnClose.addEventListener('click', hide);
  }
  return { init };
})();

TaskCompletionController.init();
